...
void gecko_mesh_set_device_name(void)
{
	...
	if(DeviceIsOnOffPublisher())
	{
		sprintf(device_name,"5823Pub %02x%02x",bt_address->address.addr[1],bt_address->address.addr[0]);
	}
	else
	{
		sprintf(device_name,"5823Sub %02x%02x",bt_address->address.addr[1],bt_address->address.addr[0]);
	}
